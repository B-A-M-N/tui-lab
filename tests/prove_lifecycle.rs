...
async fn start(pool: &SessionPool, app: &str, backend: &str) -> String {
    pool.start(
        "python3",
        &[app.to_string()],
        None,
...
async fn to_first_frame(pool: &SessionPool, sid: &str) -> u64 {
...
#[tokio::test]
async fn clean_exit_zero_reports_code_zero_everywhere() {
    let dir = TempDir::new().unwrap();
    let app = exit_mode_fixture(dir.path(), "clean");
    let pool = SessionPool::new();
    let sid = start(&pool, &app, "auto").await;
    let cursor = to_first_frame(&pool, &sid).await;
    let out = pool
        .with_session(Some(&sid), |sess| wait_exit(sess, 5_000))
        .await
        .expect("wait job");
    ...
    let exited = pool
        .with_session(Some(&sid), |sess| event_kinds(sess))
        .await
        .expect("events job")
        .into_iter()
...
#[tokio::test]
async fn nonzero_exit_preserves_the_code() {
    let dir = TempDir::new().unwrap();
    let app = exit_mode_fixture(dir.path(), "nonzero");
    let pool = SessionPool::new();
    let sid = start(&pool, &app, "auto").await;
    to_first_frame(&pool, &sid).await;
    let out = pool
        .with_session(Some(&sid), |sess| wait_exit(sess, 5_000))
        .await
        .expect("wait job");
    assert!(out.met);
    let proc = pool
        .with_session(Some(&sid), |sess| sess.process())
        .await
        .expect("process job");
...
#[tokio::test]
async fn signal_death_reports_the_signal_not_a_code() {
    let dir = TempDir::new().unwrap();
...
/// truth agree.
#[tokio::test]
async fn harness_signal_action_kills_and_is_observed() {
    let dir = TempDir::new().unwrap();
...
#[tokio::test]
async fn stop_reaps_outliving_descendants_via_the_process_group() {
    let dir = TempDir::new().unwrap();
    ...
    pool.stop(&sid).await.expect("stop");
...
#[tokio::test]
async fn running_process_state_matches_procfs() {
    let dir = TempDir::new().unwrap();
    let app = longlived_fixture(dir.path());
    let pool = SessionPool::new();
    let sid = start(&pool, &app, "auto").await;
...
/// generations.
#[tokio::test]
async fn restart_is_same_session_new_generation_new_pid() {
    let dir = TempDir::new().unwrap();
    let app = longlived_fixture(dir.path());
    let pool = SessionPool::new();
    let sid = start(&pool, &app, "auto").await;
    to_first_frame(&pool, &sid).await;
    let (old_pid, gen1) = pool
        .with_session(Some(&sid), |sess| (sess.process().pid, sess.generation))
        .await
        .expect("probe job");
    let old_pid = old_pid.expect("pid");
    let (_id, gen2) = pool.restart(&sid).await.expect("restart");
    assert_eq!(gen2, gen1 + 1, "generation increments in place");
    assert!(pool.list().contains(&sid), "restart keeps the session id");
    ...
    let out = pool
        ...
        .await
        .expect("banner wait job");
    assert!(out.met, "the restarted process prints again");
    let new_pid = pool
        .with_session(Some(&sid), |sess| sess.process().pid)
        .await
        .expect("process job")
        .expect("new pid");
...
#[tokio::test]
async fn stop_is_terminal_and_idempotent_at_the_manager() {
    let dir = TempDir::new().unwrap();
    let app = longlived_fixture(dir.path());
    let pool = SessionPool::new();
    let sid = start(&pool, &app, "auto").await;
    ...
    let pid = pool
        ...
        .expect("process job")
        .expect("pid");
    pool.stop(&sid).await.expect("first stop");
    assert!(!pool.list().contains(&sid), "session removed");
    assert!(pool.active_id().is_none(), "active pointer cleared");
    pool.stop(&sid).await.ok(); // idempotent second stop — no error
...
#[tokio::test]
async fn sighup_death_is_distinguished_from_sigterm() {
    let dir = TempDir::new().unwrap();
    let app = exit_mode_fixture(dir.path(), "sigterm");
    let pool = SessionPool::new();
    let sid = start(&pool, &app, "auto").await;
