...
impl Session {
    ...
    pub fn events_since(&self, cursor: u64) -> crate::events::EventBatch {
        self.event_state.queue().since(cursor)
    }
    ...
    pub fn events_for_consumer(&mut self, consumer: &str) -> crate::events::EventBatch {
        let cursor = self.event_state.cursor(consumer);
        let batch = self.event_state.queue().since(cursor);
        self.event_state.set_cursor(consumer, batch.cursor);
        batch
    }
    ...
    pub fn event_queue_stats(&self) -> serde_json::Value {
        serde_json::json!({
            "total": self.event_state.queue().total(),
            "retained": self.event_state.queue().retained(),
            "evicted": self.event_state.queue().evicted(),
            "last_seq": self.event_state.queue().last_seq(),
        })
    }
    ...
    pub fn drain_events(&mut self) -> Vec<crate::events::TerminalEvent> {
        self.event_state.queue_mut().drain()
    }
    ...
    pub fn event_queue_last_seq(&self) -> u64 {
        self.event_state.queue().last_seq()
    }
}
